...
@section('icerik')
<div class="box">
<div class="table-baslik" style="cursor: default;">Stok Çıkışları <button class="button tooltip" style="margin-top: -5px;" onclick="tarih(); ac('add');"><i class="fal fa-plus"></i><span class="tooltiptext">Çıkış Ekle</span></button></div>
<table id="cikisTable">
	<thead>
		<tr>
			<th>İşlem No</th>
	...
	<tbody>
		@foreach($stok_cikislar as $cikis)
		<tr class="td">
			<td>{{$cikis->islem_no}}</td>
			<td>{{$depolar->where('id', $cikis->depo_no)->value('depo_adi');}}</td>
			<td>{{$islem_tipleri->where('id', $cikis->islem_tipi_no)->value('islem_tipi');}}</td>
			<td>{{$cikis->tarih}}</td>
			<td>{{$firmalar->where('id', $cikis->firma_no)->value('firma_adi');}}</td>
			<td>{{$cikis->aciklama}}</td>
			<td class="tdbutton"><button class="button tooltip" onclick="ac('edit-{{$cikis->id}}')"><i class="fal fa-edit"></i><span class="tooltiptext">Düzenle</span></button><form action="{{ route('stok-cikisi.destroy', $cikis->islem_no)}}" method="post">@csrf @method('DELETE')<button class="button tooltip" type="submit"><i class="fal fa-trash-alt"></i><span class="tooltiptext">Sil</span></button></form></td>
		</tr>
		@endforeach
	</tbody>
</table>
...
@foreach($stok_cikislar as $cikis)
<div class="editbox" id='edit-{{$cikis->id}}'>
	<h1 class="editbaslik">Düzenle</h1><i class="fal fa-times kapat" onclick="kapa('edit-{{$cikis->id}}')"></i>
	<br />
	<form autocomplete="off" method="post" action="{{ route('stok-cikisi.update', $cikis->islem_no) }}">
	@method('PATCH')
  @csrf
		<input id="islemno_hidden-{{$cikis->id}}" type="hidden" name="islem_no" value="{{$cikis->islem_no}}"></input>
		<label class="editlabel">İşlem No: <input id="islemno-{{$cikis->id}}" disabled class="editinput" type="text" value="{{$cikis->islem_no}}"></input></label>
		<label class="editlabel">Depo: <select required class="editinput" name="depo_no">
			<option value=""></option>
			@foreach($depolar as $depo)
			<option @if ($depo->id == $depolar->where('id', $cikis->depo_no)->value('id')) selected @endif value="{{$depo->id}}">{{$depo->depo_adi}}</option>
			@endforeach
		</select>
		</label>
		<label class="editlabel">Çıkış Tipi: <select required class="editinput" onchange="selectCikis(this, '-{{$cikis->id}}')" name="islem_tipi_no">
			<option value=""></option>
			@foreach($islem_tipleri as $islem)
			<option @if ($islem->id == $islem_tipleri->where('id', $cikis->islem_tipi_no)->value('id')) selected @endif onek="{{$islem->on_ek}}" deger="{{$islem->deger}}" value="{{$islem->id}}">{{$islem->islem_tipi}}</option>
			@endforeach
		</select>
		</label>
		<label class="editlabel">Tarih: <input required class="editinput" type="datetime-local" value="{{$cikis->tarih}}" name="tarih"></input></label>
		<input type="hidden" id="firma_no-{{$cikis->id}}" name="firma_no" value="{{$cikis->firma_no}}"></input>
		<label class="editlabel">Firma: <input readonly id="firma_adi-{{$cikis->id}}" value="{{$firmalar->where('id', $cikis->firma_no)->value('firma_adi');}}" class="editinput" style="width: 150px;" type="text"></input><div class="karebutton" onclick="ac('firmaadd-{{$cikis->id}}')"><i class="fal fa-plus"></i></div></label>
		<label class="editlabel">Açıklama: <input class="editinput" type="text" value="{{$cikis->aciklama}}" name="aciklama"></input></label>
		<br />
		<table style="background-color: #ecf0ff; white-space:nowrap;">
			...
			<tbody id="stokadd_table-{{$cikis->id}}">
				@foreach($stok_cikislar_tum as $cikis_tum)
					@if ($cikis->islem_no == $cikis_tum->islem_no)
						<tr class="td">
							<td>{{$stoklar->where('id', $cikis_tum->stok_no)->value('stok_kodu');}}</td>
							<td>{{$cikis_tum->barkod_no}}</td>
							<td>{{$stoklar->where('id', $cikis_tum->stok_no)->value('stok_adi');}}</td>
							<td><input type='number' style="width: 45px;" name='miktar-{{$cikis_tum->sira_no}}' value="{{$cikis_tum->miktar}}"></td>
							<td>{{$birimler->where('id', $stoklar->where('id', $cikis_tum->stok_no)->value('birim_no'))->value('birim')}}</td>
							<td>{{$cikis_tum->fiyat}}</td>
							<td>{{$cikis_tum->kdv_oran}}</td>
							<td><input id="sonkullanim-{{$cikis_tum->id}}-{{$cikis_tum->sira_no}}" value='{{$cikis_tum->son_kullanim}}' style="width: 100px;" type='date' name='son_kullanim-{{$cikis_tum->sira_no}}'></td>
							<td>{{$cikis_tum->sira_no}}</td>
							<td><button onclick="stoksil(this, '-{{$cikis->id}}')" class="button tooltip"><i class="fal fa-trash-alt"></i><span class="tooltiptext">Sil</span></button></td>
							<input type='hidden' name='stok_no-{{$cikis_tum->sira_no}}' value='{{$cikis_tum->stok_no}}'>
							<input type='hidden' name='barkod_no-{{$cikis_tum->sira_no}}' value='{{$cikis_tum->barkod_no}}'>
							<input type='hidden' name='fiyat-{{$cikis_tum->sira_no}}' value='{{$cikis_tum->fiyat}}'>
							<input type='hidden' name='kdv_oran-{{$cikis_tum->sira_no}}' value='{{$cikis_tum->kdv_oran}}'>
							<input type='hidden' name='sira_no-{{$cikis_tum->sira_no}}' value='{{$cikis_tum->sira_no}}'>
						</tr>
					@endif
				@endforeach
			</tbody>
			<tfoot>
				<tr style="cursor: pointer;" onclick="ac('stokadd-{{$cikis->id}}')">
					<td></td>
					<td></td>
					<td></td>
					<td></td>
		...
		<input type="hidden" id="sira_no-{{$cikis->id}}" name="sira_no" value=""></input>
		<input id="kaydetbutton-{{$cikis->id}}" class="editbutton" type="submit" value="Kaydet"></input>
	</form>
</div>

<div class="editbox" id='firmaadd-{{$cikis->id}}'>
	<h1 class="editbaslik">Firma Seç</h1><i class="fal fa-times kapat" onclick="kapa('firmaadd-{{$cikis->id}}')"></i>
	<br />
	<table>
		<thead>
			<tr>
		...
		<tbody>
			...
			<tr class="td">
				<td style="cursor: pointer;" firmano="{{$firma->id}}" firmaadi="{{$firma->firma_adi}}" onclick="firmaSec(this, '-{{$cikis->id}}')">{{$firma->firma_adi}}</td>
			</tr>
			@endforeach
		</tbody>
	</table>
...
<div class="editbox" id='stokadd-{{$cikis->id}}'>
	<h1 class="editbaslik">Stok Seç</h1><i class="fal fa-times kapat" onclick="kapa('stokadd-{{$cikis->id}}')"></i>
	<br />
	<table>
		<thead>
			<tr>
		...
		<tbody>
			...
			<tr style="cursor: pointer;" id="{{$stok->id}}" onclick="stokSec(this, '-{{$cikis->id}}')" class="td">
...
<div class="editbox" id='add'>
	<h1 class="editbaslik">Çıkış Ekle</h1><i class="fal fa-times kapat" onclick="kapa('add')"></i>
	<br />
	<form autocomplete="off" method="post" action="{{ route('stok-cikisi.store') }}">
  @csrf
		...
		<label class="editlabel">Çıkış Tipi: <select required class="editinput" onchange="selectCikis(this, '')" name="islem_tipi_no">
			<option value=""></option>
			@foreach($islem_tipleri as $islem)
...
$(document).ready(function () {
    $('#cikisTable').DataTable();
});

@foreach($stok_cikislar as $cikis)
	document.getElementById("sira_no-{{$cikis->id}}").value = document.getElementById("stokadd_table-{{$cikis->id}}").childElementCount;
@endforeach
